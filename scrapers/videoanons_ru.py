...
class VideoAnons(OpenSearchMixin, SimpleScraperBase):
    BASE_URL = 'http://www.videoanons.ru'
    OTHER_URLS = []
    ...
    def setup(self):
        ...
        self.bunch_size = 50
        self.media_type_to_category = 'tv 0, film 0'  # 'tv 20, film 3'
        self.encode_search_term_to = 'cp1251'
        self.showposts = 0

    def _parse_search_result_page(self, soup):
        found = 0
        for res in soup.select('.newstitle a'):
    ...
    def _parse_parse_page(self, soup):
        title = soup.select_one('.newstitle').text
        ...
        for box in soup.select('#dle-content'):
            for url in self.util.find_urls_in_text(str(box)):
                if len(url) > 30 and not url.startswith(self.BASE_URL):
                    self.submit_parse_result(index_page_title=soup.title.text.strip(),
                                             link_url=url,
                                             link_title=title,
                                             series_season=series_season,
